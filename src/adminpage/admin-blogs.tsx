...
const AdminBlogs = () => {
  ...
  const handleSave = () => {
    ...
    fetch("http://192.168.55.114/tara-kabataan/tara-kabataan-backend/api/update_blogs.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch("http://192.168.55.114/tara-kabataan/tara-kabataan-backend/api/upload_blog_image.php", {
        method: "POST",
        body: formData,
      });
      const data = await res.json();
  ...
  const confirmDeleteBlog = () => {
    ...
    fetch("http://192.168.55.114/tara-kabataan/tara-kabataan-backend/api/delete_blogs.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const handleContentImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.success && data.image_url) {
        const imageTag = `<img src="http://192.168.55.114${data.image_url}" alt="inserted image" style="max-width:100%;" />`;
  ...
  const applyBulkStatus = async (newStatus: string) => {
    ...
    try {
      const response = await fetch("http://192.168.55.114/tara-kabataan/tara-kabataan-backend/api/update_bulk_blog_status.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleBulkDelete = async () => {
    try {
      const response = await fetch("http://192.168.55.114/tara-kabataan/tara-kabataan-backend/api/delete_bulk_blogs.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  useEffect(() => {
    fetch(`http://192.168.55.114/tara-kabataan/tara-kabataan-backend/api/get_user_name.php?user_id=${newBlogAuthor}`)
      .then(res => res.json())
      .then(data => {
        if (data.user_name) {
  ...
  const handleNewBlogImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch("http://192.168.55.114/tara-kabataan/tara-kabataan-backend/api/add_new_blog_image.php", {
        method: "POST",
        body: formData,
      });
  ...
  const handleNewBlogSave = async () => {
    ...
    try {
      const res = await fetch("http://192.168.55.114/tara-kabataan/tara-kabataan-backend/api/add_new_blog.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const togglePinBlog = (blogId: string, currentPinned: boolean) => {
    ...
    fetch("http://192.168.55.114/tara-kabataan/tara-kabataan-backend/api/update_blog_pin_status.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  useEffect(() => {
    if (selectedBlog) {
      fetch(`http://192.168.55.114/tara-kabataan/tara-kabataan-backend/api/get_blog_images.php?blog_id=${selectedBlog.blog_id}`)
        .then(res => res.json())
        .then(data => {
            ...
            <div className="blog-grid-scrollable-wrapper">
              <div className="blog-grid-container">
                {filteredBlogs.map((blog) => (
                  ...
                  >
                    <img
                      src={`http://192.168.55.114${blog.image_url}`}
                      alt={blog.title}
                  ...
                  <div className="admin-blogs-modal-right">
                    <div className="admin-blogs-modal-image">
                      ...
                      {(isEditing ? editableBlog?.image_url : selectedBlog.image_url) ? (
                        <img
                        src={`http://192.168.55.114${isEditing ? editableBlog?.image_url : selectedBlog.image_url}`}
                        alt="Blog"
                        style={{ cursor: "zoom-in" }}
                        onClick={() =>
                          setFullImageUrl(`http://192.168.55.114${isEditing ? editableBlog?.image_url : selectedBlog.image_url}`)
                        }
                      />
                        ...
                        {(isEditing ? editableBlogMoreImages : selectedBlog.more_images).slice(0, 4).map((img, i) => {
                          ...
                          return (
                            <div key={i} className="blog-image-preview">
                              <img
                                src={`http://192.168.55.114${img}`}
                                alt={`More Image ${i}`}
                                onClick={() => setFullImageUrl(`http://192.168.55.114${img}`)}
                                style={{ cursor: "zoom-in" }}
                              />
                              {isLast && (
                                <div
              ...
              <div className="admin-blogs-new-blog-modal-inner-content">
                <div className="admin-blogs-new-blog-modal-inner-content-top">
                  ...
                  <div className="admin-blogs-new-blog-modal-image">
                    ...
                    {newBlogImage ? (
                      <img
                      src={`http://192.168.55.114${newBlogImage}`}
                      alt="Preview"
                      style={{ maxWidth: "100%", maxHeight: "300px", borderRadius: "4px", cursor: "zoom-in" }}
                      onClick={() => setFullImageUrl(`http://192.168.55.114${newBlogImage}`)}
                    />                    
                    ) : (
                      <div
                        style={{
                        ...
                        <input
                          ...
                          onChange={async (e) => {
                            ...
                            try {
                              ...
                              const data = await res.json();
                              if (data.success && data.image_url) {
                                const img = `<img src="http://192.168.55.114${data.image_url}" alt="blog image" style="max-width:100%;" />`;
            ...
            {(
              ...
              >
                <div
                  ...
                  onClick={() => setFullImageUrl(`http://192.168.55.114${img}`)}
                >
                  <img src={`http://192.168.55.114${img}`} alt={`More Image ${index}`} />
