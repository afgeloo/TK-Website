...
const AdminLogin: React.FC = () => {
  ...
  const [resetStep, setResetStep] = useState<"method" | "email" | "otp" | "newpass">("method");
  ...
  const phoneOtpRefs = Array(6).fill(null).map(() => React.createRef<HTMLInputElement>());
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("http://172.20.10.2/tara-kabataan/tara-kabataan-backend/api/adminlogin.php", {
        method: "POST",
      ...
      if (data.success) {
        ...
        if (simulateOTP) {
          ...
          if (isPhoneLogin) {
            ...
            const otpRes = await fetch("http://172.20.10.2/tara-kabataan/tara-kabataan-backend/api/send_phone_otp.php", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
            ...
            const otpRes = await fetch("http://172.20.10.2/tara-kabataan/tara-kabataan-backend/api/send_otp.php", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
  ...
  const handlePasswordReset = async () => {
    ...
    try {
      const res = await fetch("http://172.20.10.2/tara-kabataan/tara-kabataan-backend/api/resetadminpass.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: resetContact, new_password: newPassword }),
      });
        ...
        {isPhoneLogin ? (
          ...
          {isLocked && (
            <div className="admin-login-error">
              Locked out. Please wait {lockRemaining} second{lockRemaining !== 1 && 's'}.
            </div>
          )}
            ...
            <div className="otp-box-wrapper">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const res = await fetch("http://172.20.10.2/tara-kabataan/tara-kabataan-backend/api/verify_phone_otp.php", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
        ...
        <div className="reset-password-modal">
          ...
          {resetStep === "method" && (
            ...
            <div className="modal-close-icon" onClick={closeResetModal}            >
              ...
              <div className="modal-buttons">
                <button className="admin-login-button" onClick={() => setResetStep("email")}>
                  Reset via Email
                </button>
          ...
          {resetStep === "otp" && (
            ...
            <div className="modal-close-icon" onClick={closeResetModal}>
              ...
              {resetOtpError && <div className="admin-login-error">{resetOtpError}</div>}
            </div>
              <div className="modal-buttons">
                <button onClick={async () => {
                  if (resetOtpLocked) {
