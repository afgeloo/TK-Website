...
function SingleBlog() {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        const response = await fetch(`http://192.168.55.114/tara-kabataan/tara-kabataan-backend/api/blogs.php?blog_id=${id}`);
  ...
  useEffect(() => {
    const fetchMoreImages = async () => {
      try {
        const response = await fetch(`http://192.168.55.114/tara-kabataan/tara-kabataan-backend/api/get_blog_images.php?blog_id=${id}`);
        const data = await response.json();
  ...
  const getFullImageUrl = (path: string): string => {
    return path.startsWith("/") 
      ...
      : `http://192.168.55.114/tara-kabataan/tara-kabataan-webapp/uploads/blogs-images/${path}`;
  };  
